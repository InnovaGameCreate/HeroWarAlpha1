...
namespace Unit
{
    public class CharacterSearchVer2 : MonoBehaviour
    {
        [Header("参照元")]
        [SerializeField]
        ...
        private void Awake()
        {
            ...
            MyCharacterProfile
                ...
                .Subscribe(_ =>
                {
                    if (!isInit)
                    {
                        Debug.Log("CharacterSearchVer2:初期値の設定がされました");
                        Init();
                    }
                })
                .AddTo(this);
        ...
        IEnumerator SearchSystemloop()
        {
            while (true)
            {
                ...
                if (MyCharacterProfile.isHasInputAuthority()) Debug.Log($"CharacterSearchVer2の各値について。allEnemy = {allEnemy.Count} : inAttackRangeEnemy = {inAttackRangeEnemy.Count}");
                //Listの整理
                resetList();
            }
        }
